...
void PrintList(std::ostream& out,   // REC'D/P'BACK: ouput stream
	       SortedType& list); // REC'D/P'BACK: list to be printed
...
int main(int argc, char *argv[])
{
   ...
   int numItems;
   cout << "Enter the number of items for the list-->" << endl;
   cin >> numItems;
   
   int number;
   ItemType item;
   SortedType list (numItems);
   // call the explicit value constructor with size = numItems
   while (inFile >> number)
   {
...
void PrintList(std::ostream& out, SortedType& list)
{
   using std::endl;
