  ...
  if ((row < numRows && row >= 0) && (col < numColumns && col >= 0))
    {
      ...
      //a loop to see if any of the cell is open or not
      for (int row = 0; row < numRows; row++)
	{
	  for (int col = 0; col < numColumns; col++)
	    {
      ...
      //uncovers the current row and col and stores th bool
      found =  gridArray[Place(row,col)].Uncover();

      //if it is the first and has a bomb
      //then remove it and place it in a diferent cell
      //adjust the number of bombs accordingly
      while (found && first == 0)
	{
	  RemoveBomb(row,col);
      ...
      //A test to check if the adjacent cells have bomb in them or not
      int test = 0;
      //A test to check if all the cell have been uncovred or not
      int test2 = 0;

      //checks to see if the cells have bomb in them
      if ((i-1) >= 0
	  && !gridArray[Place (i-1,j)].HasBomb())
	test++; 
      if ((i+1) < numRows
	...
	test++;
      ...
      //checks to see if the cells surrounding is uncovered or not
      if ((i-1) >= 0	 
	...
	test2++;
      ...
      //if the cells are in a corner or are on the edges
      //then the num of adjacent decreases accordingly
      if (((row == 0 && col == 0) ||
      ...
      //Go in the loop of the surrounding cells have no bombs
      if(test == 8)
	{
	  //uncivers the cell
	  UncoverRecursive(row, col);
	  //checks to see if the surrounding cells have been uncovered or not
	  //if yes then calls this function Uncover again to go through the grid
