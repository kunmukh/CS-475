...
// Post: returns the number of nodes in the tree.
{
  if (tree == NULL) //base case :empty tree
    return 0;
  // Recursive calls, i.e., smaller caller
  int leftCount = CountNodes (tree->left),
    rightCount = CountNodes (tree->right);
  //General case: sum of the subtree +1 for this node;
...

{
  ...
  //Iitialize scanning pointer
  parentPtr = NULL;
  nodePtr = tree;
  bool found = false; // set to true if find the item
  //loop while there are nodes to visit and not found
  while(nodePtr != NULL && !found)
    {
      //check if need to go down left side
      if(item < nodePtr->info)
	{
	  parentPtr = nodePtr;
	  nodePtr = nodePtr->left;
	}
      //check if need to go down the right side
...
// Post: item is in tree.
{
  ...
  //CHeck if this is the first node inserted
  if (parentPtr == NULL)
    root = newNode;
  // See if it goes on the left side
  else if (item < parentPtr->info)
    parentPtr->left = newNode;
  //Otherwise goes to the right side
  //else if (item > parentPtr->info)
  else
