...
void Document::MoveCurrent (int n)
{
  if (!IsEmpty())
	...
	throw (std::string("Invalid number"));    
      ...
      try
	{	      
	  //set the current node up or down that many
	  //placed depending on the user input
	  SetCurrent (length + n);
	}
      //else catch the string excepton and throw it bakc
  ...
  if (!IsEmpty())
    {
      while (temp != headPtr)
	{
	  //kepp printout the info to the output stream
